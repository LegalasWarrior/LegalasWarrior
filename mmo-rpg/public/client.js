...
function renderAll() {
  ...
  if (isBusy()) {
    ...
    div.textContent = `⏳ ${you.busy.action || 'Занято'} (${remain}с)`;
    logEl.appendChild(div);
  }

  // Combat
  if (you.encounter) { combatPanel.style.display = ''; combatInfo.textContent = `${you.encounter.name} — HP ${you.encounter.hp}`; }
  else { combatPanel.style.display = 'none'; }
  ...
  btnHeal.disabled = isBusy() || you.location !== 'city';
  ...
  for (const [key, label] of equipSlots) {
    const row = document.createElement('div'); row.className = 'list';
    const val = you.equipment[key]; const name = val ? itemName(val) : '—';
...
function renderInvRow(k, cnt, where, meta) {
  ...
  if (itType === 'weapon' || itType === 'armor' || itType === 'tool') {
    const be = document.createElement('button'); be.textContent = 'Экипировать'; be.disabled = isBusy(); be.onclick = () => send('equip', { itemKey: k }); box.appendChild(be);
  }
  if (itType === 'consumable') {
    const bu = document.createElement('button'); bu.textContent = 'Использовать'; bu.disabled = isBusy(); bu.onclick = () => send('use', { itemKey: k }); box.appendChild(bu);
  }

  if (where === 'inv') {
    const bStore = document.createElement('button'); bStore.textContent = 'На склад'; bStore.disabled = isBusy(); bStore.onclick = () => numberPrompt(`Сколько переместить на склад? (1-${cnt})`, (q) => send('store', { itemKey: k, qty: q })); box.appendChild(bStore);
  } else if (where === 'storage') {
    const bTake = document.createElement('button'); bTake.textContent = 'Забрать'; bTake.disabled = isBusy(); bTake.onclick = () => numberPrompt(`Сколько забрать? (1-${cnt})`, (q) => send('withdraw', { itemKey: k, qty: q })); box.appendChild(bTake);
  }
...
function renderCraft(meta, you) {
  craftEl.innerHTML = '';
  const info = document.createElement('div'); info.className = 'small'; info.textContent = 'Крафт возможен в городе и ремесленных угодьях.'; craftEl.appendChild(info);
  for (const r of meta.recipes) {
    ...
    const btn = document.createElement('button'); btn.textContent = 'Создать'; btn.disabled = isBusy(); btn.onclick = () => numberPrompt('Сколько создать?', (q) => send('craft', { recipeKey: r.key, qty: q })); row.appendChild(btn);
